...
def prepare_code(code):
    ...
    return [
        map_bracket(c, i) if c in ("[", "]") else c
...
def eval_step(code, data, code_pos, data_pos, out):
    c = code[code_pos]
    d = data[data_pos]
    step = 1
    ...
    return (data, code_pos, data_pos, step, out)
...
def eval(code, data=[0 for i in range(9999)], c_pos=0, d_pos=0):
    ...
    while c_pos < len(code):
        out = ""
        (data, c_pos, d_pos, step, out) = eval_step(code, data, c_pos, d_pos, out)
        result += out
        c_pos += step
    return result
